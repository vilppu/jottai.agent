...
module Application =
    ...
    let HasTokenSecret =
        let tokenSecret =
            Environment.GetEnvironmentVariable("JOTTAI_TOKEN_SECRET")
    ...
    let Authority () : string =
        let authority =
            Environment.GetEnvironmentVariable("JOTTAI_AUTHORITY")
    ...
    let Audience () : string =
        let audience =
            Environment.GetEnvironmentVariable("JOTTAI_AUDIENCE")
    ...
    let ClientId () : string =
        let clientId =
            Environment.GetEnvironmentVariable("JOTTAI_CLIENT_ID")
    ...
    let ManagementClientId () : string =
        let authority =
            Environment.GetEnvironmentVariable("JOTTAI_MANAGEMENT_CLIENT_ID")
    ...
    let ManagementClientSecret () : string =
        let authority =
            Environment.GetEnvironmentVariable("JOTTAI_MANAGEMENT_CLIENT_SECRET")
    ...
    let ManagementAudience () : string =
        let authority =
            Environment.GetEnvironmentVariable("JOTTAI_MANAGEMENT_AUDIENCE")
    ...
    let GenerateDeviceGroupId () : string =
        ...
        let token =
            tokenWithDashes.Replace("-", "").ToLower()
    ...
    let PostSensorName deviceGroupId propertyId propertyName : Async<unit> =
        async {
            ...
            | Some propertyName ->
                ...
                let command =
                    Command.ChangeSensorName changeSensorName
    ...
    let GetSensorStates (deviceGroupId: string) : Async<ApiObjects.SensorState list> =
        async {
            ...
            return statuses |> ConvertSensorState.ToApiObjects
        }
    ...
    let GetSensorHistory (deviceGroupId: DeviceGroupId) (propertyId: PropertyId) : Async<ApiObjects.SensorHistory> =
        async {
            ...
            let result =
                history |> ConvertSensorHistory.ToApiObject
    ...
    let GetDeviceProperties (deviceGroupId: DeviceGroupId) : Async<ApiObjects.DeviceProperty list> =
        async {
            ...
            let result =
                commands |> ConvertDeviceProperty.ToApiObjects
    ...
    let SubscribeToPushNotifications deviceGroupId (token: string) : Async<unit> =
        async {
            ...
            let command =
                Command.SubscribeToPushNotifications subscribeToPushNotifications
    ...
    let GetDevicePropertyChangeRequest (deviceGroupId: string) : Async<ApiObjects.DevicePropertyChangeRequest option> =
        async {
            ...
            do! EventBus.Publish Event.PollingDevicePropertyChangeRequests

            let! devicePropertyChangeRequest = WaitForDevicePropertyChangeRequestedEvent.For deviceGroupId
    ...
    type private EventSubscriptions(subscriptions: List<IDisposable>) =
    ...
    let StartProcessingEvents httpSend : IDisposable =
        new EventSubscriptions(
            [ SensorEventHandler.Handle EventBus.Publish
              |> EventBus.Subscribe
              DevicePropertyEventHandler.Handle EventBus.Publish
              |> EventBus.Subscribe
              PushNotificationEventHandler.Handle httpSend EventBus.Publish
              |> EventBus.Subscribe ]
        )
        :> IDisposable
